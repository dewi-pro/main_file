        ...
        <div class="m-auto col-lg-8">
            <div class="card">
                <div class="card-header">
                    <h5> {{ __('Appoinment Seat Wise') }}</h5>
                </div>
                <form action="{{ route('booking.fill.store', $booking->id) }}" method="POST" enctype="multipart/form-data"
                    id="fill-form">
                    @method('PUT')
                    @csrf
                    <div class="card-body">
...
@push('script')
    ...
    <script src="https://www.paypal.com/sdk/js?client-id={{ Utility::getsettings('PAYPAL_SANDBOX_CLIENT_ID') }}&currency={{ $booking->currency_name }}">
    </script>
    ...
    <script>
        $(document).ready(function() {
            ...
            $(document).on('click', 'input[name="seat[]"]', function() {
                var limitSlots = parseInt({{ $seatWiseBooking->maximum_limit }});
