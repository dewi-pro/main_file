...
class FormCategoryDataTable extends DataTable
{
    /**
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return datatables()
            ->eloquent($query)
            ->addIndexColumn()
            ->addColumn('action', function (FormCategory $category) {
                return view('form-category.action', compact('category'));
            })
            ->editColumn('created_at', function ($request) {
                return UtilityFacades::date_time_format($request->created_at);
            })->editColumn('status', function (FormCategory $category) {
                $checked = ($category->status == 1) ? 'checked' : '';
                $status   = '<label class="form-switch">
                         <input class="form-check-input chnageStatus" name="custom-switch-checkbox" ' . $checked . ' data-url="' . route('formcategory.status', $category->id) . '" type="checkbox">
                         </label>';
                return $status;
            })
     ...
     *
     * @param \App\Models\FormCategory $model
     * @return \Illuminate\Database\Eloquent\Builder
     */
    public function query(FormCategory $model): QueryBuilder
    {
    ...
    public function html(): HtmlBuilder
    {
        ...
        $canCreateCategory = \Auth::user()->can('create-form-category');
    ...
    {
        return 'FormCategory_' . date('YmdHis');
    }
}
