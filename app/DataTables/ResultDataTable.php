...
class ResultDataTable extends DataTable
{
    public function dataTable($query)
    {
        return datatables()
            ...
            ->editColumn('created_at', function (Form $form) {
                ...
                return $category;
            })->editColumn('destination', function (Form $form) {
                $cluster = $form->destination;
                return $cluster;
            })
    ...
    public function query(Form $model, Request $request)
    {
        ...
        if ($request->categories) {
            $form->whereIn('forms.category_id', $request->categories);
        }
        if ($request->clusters) {
            $form->whereIn('forms.cluster_id', $request->clusters);
        }
        if ($request->leaders) {
            $form->whereIn('forms.leader_id', $request->leaders);
        }
        if ($request->role) {
