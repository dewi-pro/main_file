...
class AnnouncementDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return datatables()
            ...
            ->editColumn('image', function (Announcement $announcement) {
                if ($announcement->image) {
                    if (Storage::url($announcement->image)) {
                        $return = "<img src='" . Storage::url($announcement->image) . "' width='50' />";
                    } else {
            ...
            ->addColumn('action', function (Announcement $announcement) {
                return view('announcement.action', compact('announcement'));
            })
            ->editColumn('status', function (Announcement $announcement) {
                $checked = ($announcement->status == 1) ? 'checked' : '';
                $status   = '<label class="form-switch">
                         <input class="form-check-input chnageStatus" name="custom-switch-checkbox" ' . $checked . ' data-id="' . $announcement->id . '" data-url="' . route('announcement.status', $announcement->id) . '" type="checkbox">
                         </label>';
                return $status;
            })
    ...
    public function html(): HtmlBuilder
    {
        ...
        $canCreateAnnouncement = \Auth::user()->can('create-announcement');
        $canExportAnnouncement = \Auth::user()->can('export-announcement');
        $buttonsConfig = [];
        if ($canCreateAnnouncement) {
            $buttonsConfig[] =  [
                ...
                'action' => " function ( e, dt, node, config ) {
                    window.location = '" . route('announcement.create') . "';
                }"
            ];
        }
